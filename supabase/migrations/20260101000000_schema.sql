-- GENERATED COPY of infra/schema.sql. DO NOT EDIT THIS FILE.
--
-- The Supabase CLI only applies migrations from supabase/migrations/, and infra/schema.sql is
-- what a human pastes into the SQL editor for a fresh hosted project. Both have to exist, so this
-- is a copy — and a copy that drifts is worse than no copy at all, because CI would then be
-- testing a schema the real project does not have.
--
-- The "schema files match" step in .github/workflows/tests.yml fails the build if they diverge.
-- To change the schema: edit infra/schema.sql, then
--     cp infra/schema.sql supabase/migrations/20260101000000_schema.sql
--
